...
def create_unary_node(tokens):
    sign = tokens[0]
    operand_val = tokens[1]
    if sign == '-':
        return UnaryOpNode('-', operand_val)
    return operand_val

nested_expr = ("(" + expr + ")").setParseAction(lambda tokens: tokens[1])
unary_expr = (oneOf("+ -") + (function_call | var | number | nested_expr)).setParseAction(create_unary_node)

# Operands
operand = unary_expr | function_call | var | number | nested_expr
